...
## <a name="configure-at-least-one-distribution-point-to-accept-pxe-requests"></a><a name="BKMK_Configure"></a> PXE isteklerini kabul etmek için en az bir dağıtım noktası yapılandırma

PXE önyükleme istekleri yapan Configuration Manager istemcilere işletim sistemlerini dağıtmak için PXE isteklerini kabul etmek üzere bir veya daha fazla dağıtım noktası yapılandırmanız gerekir. Dağıtım noktasını yapılandırdıktan sonra, PXE önyükleme isteklerine yanıt verir ve yapılacak uygun dağıtım eylemini belirler. Daha fazla bilgi için, bkz. [Install or modify a distribution point](../../core/servers/deploy/configure/install-and-configure-distribution-points.md#bkmk_config-pxe).  

> [!NOTE]  
> Birden çok alt ağı desteklemek için tek bir PXE etkin dağıtım noktası yapılandırırken, DHCP seçeneklerini kullanmak desteklenmez. PXE isteklerinin PXE 'yi destekleyen dağıtım noktalarına iletilmesine izin vermek için yönlendiricilerdeki IP Yardımcıları yapılandırın.
>
...
Bir işletim sistemini dağıtmak üzere PXE kullanmak için, bir veya daha fazla PXE 'yi destekleyen dağıtım noktasına dağıtılmış x86 ve x64 PXE özellikli önyükleme görüntülerinin olması gerekir. PXE’yi bir önyükleme görüntüsünde etkinleştirmek ve önyükleme görüntüsünü dağıtım noktalarına dağıtmak için bu bilgileri kullanın:
...
- Önyükleme görüntüsünün özelliklerini değiştirirseniz önyükleme görüntüsünü dağıtım noktalarına güncelleştirin ve yeniden dağıtın. Daha fazla bilgi için, bkz. [Distribute content](../../core/servers/deploy/configure/deploy-and-manage-content.md#bkmk_distribute).
...
### <a name="process-to-create-the-exclusion-list"></a>Dışlama listesini oluşturma işlemi

1. PXE için etkinleştirilmiş bir dağıtım noktasında bir metin dosyası oluşturun. Örneğin, bu metin dosyasını **pxeExceptions.txt**olarak adlandırın.  

2. Not Defteri gibi bir düz metin düzenleyicisi kullanın ve PXE 'yi destekleyen dağıtım noktası tarafından yoksayılacak bilgisayarların MAC adreslerini ekleyin. MAC adresi değerlerini virgüllerle ayırın ve her adresi ayrı bir satıra girin. Örneğin, `01:23:45:67:89:ab`  

3. Metin dosyasını PXE'yi destekleyen dağıtım noktası site sistem sunucusuna kaydedin. Metin dosyası, sunucudaki herhangi bir konuma kaydedilebilir.  

4. Bir **MACIgnoreListFile** kayıt defteri anahtarı oluşturmak için PXE 'yi destekleyen dağıtım noktasının kayıt defterini düzenleyin. PXE 'yi destekleyen dağıtım noktası site sistem sunucusundaki metin dosyasının tam yolunun dize değerini ekleyin. Aşağıdaki kayıt defteri yolunu kullanın:  
...
5. Bu kayıt defteri değişikliğini yaptıktan sonra WDS hizmetini veya PXE Yanıtlayıcı hizmetini yeniden başlatın. Sunucuyu yeniden başlatmanız gerekmez.<!--512129-->  
...
Sürüm 1906 ' den başlayarak PXE DHCP el sıkışması sırasında 82 seçeneği WDS olmadan PXE Yanıtlayıcı ile desteklenir. 82 seçeneği gerekliyse, PXE Yanıtlayıcı 'yı WDS olmadan kullandığınızdan emin olun. 82 seçeneği WDS ile desteklenmez.
...
> [!IMPORTANT]  
> PXE protokolü güvenli değil. PXE sunucusunun ve PXE istemcisinin, sitenize yetkisiz erişimi engellemek için bir veri merkezinde olduğu gibi fiziksel olarak güvenli bir ağda bulunduğundan emin olun.
