...
CA 'lardan CMPivot çalıştırdığınızda, birincil siteyle iletişim kurmak için yüksek hızlı ileti abonelik kanalını kullanır. CMPivot, siteler arasında standart SQL çoğaltma kullanmaz. SQL Server Örneğiniz veya SQL sağlayıcınız uzak ise veya SQL Server her zaman açık kullanıyorsanız, CMPivot için bir "çift atlama senaryosu" olur. "Çift atlama senaryosu" için kısıtlanmış temsilciyi tanımlama hakkında daha fazla bilgi için bkz. [sürüm 1902 ' den başlayarak CMPivot](cmpivot.md#bkmk_cmpivot1902).
...
Varsayılan olarak, site sunucusu günlük dosyaları konumunda `C:\Program Files\Microsoft Configuration Manager\logs`bulunur. Varsayılan olmayan bir yükleme dizini veya başka bir sunucuya SMS sağlayıcısı gibi boşaltılan öğeler belirttiyseniz bu konum farklı olabilir. CA 'lardan CMPivot çalıştırırsanız, Günlükler birincil site sunucusudur.
...
- Configuration Manager sürüm 1902:
 ...
 `7DC6B6F1-E7F6-43C1-96E0-E1D16BC25C14`, CMPivot için komut dosyası GUID 'Sidir. Bu GUID 'yi, [CMPivot denetim durumu iletilerinde](cmpivot.md#cmpivot-audit-status-messages)de görebilirsiniz.
...
`TaskID` Clientaction tablosundan öğesini bulun. , `TaskID` Clientaction tablosundaki `UniqueID` öğesine karşılık gelir.

``` SQL
select * from ClientAction where ClientOperationId=<id>
```

İçinde `BgbServer.log`, SQL 'den toplanan `TaskID` ve ' a göz atın `PushID`. , `TaskID` Etiketlidir `TaskGUID`. Örneğin:
...
Site sunucusundan bilgi aldıktan sonra istemci günlüklerine bakın. Varsayılan olarak, istemci günlükleri ' de `C:\Windows\CCM\Logs`bulunur.

' `CcmNotificationAgent.log`De, aşağıdaki satırlar gibi görünen günlük girdilerini arayın:  
...
İçin `Scripts.log` olup olmadığını `TaskID`denetleyin. Aşağıdaki örnekte şunları görürsünüz `Task ID` `{9A4E59D2-2F5B-4067-A9FA-B99602A3A4A0}`:  
...
> [!NOTE]
> İçinde "(hızlı)" `Scripts.log`öğesini görmüyorsanız, veriler büyük OLASıLıKLA 80 KB 'tan fazla olur. Bu durumda, bilgiler site sunucusuna durum iletisi olarak gönderilir. İstemci `StateMessage.log` ve site sunucusu ' nı kullanın `Statesys.log`.
...
`MP_RelayMsgMgr.log` Örnekte, istemcinin kimliğini `(GUID:83F67728-2E6D-4E4F-8075-ED035C31B783)` ve ' u görebilirsiniz `Task ID {9A4E59D2-2F5B-4067-A9FA-B99602A3A4A0}`. İleti işleme altyapısına gönderilmeden önce istemcinin yanıtına bir ileti KIMLIĞI atanır:
...
> [!TIP]
> İşlem sırasında bir özel durum alırsanız, aşağıdaki SQL sorgusunu çalıştırarak ve özel durum sütununa bakarak onu gözden geçirebilirsiniz. İleti işlendikten sonra artık `MPE_RequestMessages_Instant` tabloda yer olmayacaktır.
>
> ```SQL
> select * from MPE_RequestMessages_Instant where MessageID=<ID from SMS_MESSAGE_PROCESSING_ENGINE.log>
> ```

İçinde `BgbServer.log`, bildirilen veya başarısız `PushID` olan istemci sayısını görmek için öğesine bakın.
...
Varsayılan olarak, site sunucusu günlük dosyaları konumunda `C:\Program Files\Microsoft Configuration Manager\logs`bulunur. Varsayılan olmayan bir yükleme dizini veya başka bir sunucuya SMS sağlayıcısı gibi boşaltılan öğeler belirttiyseniz bu konum farklı olabilir.

Bu satır `smsprov.log` için bakın:
...
İçinde `BgbServer.log`, SQL 'den toplanan `TaskID` öğesine bakın. Etiketlendi `TaskGUID`. Örneğin:
...
' `CcmNotificationAgent.log`De, aşağıdaki girdiye benzer Günlükler arayın:  
...
`Scripts.log` İçin bölümüne bakın `TaskID`. Aşağıdaki örnekte şunları görüyoruz `Task ID {F8C7C37F-B42B-4C0A-B050-2BB44DF1098A}`:
...
İletinin `statesys.log` alınıp işlenip işlenmediğini görmek için açın. Aşağıdaki örnekte, öğesinin yanında iletinin alt `TaskID` kısmına yakın bir ileti görürsünüz `<Param>`. Bu günlük girişlerini görmek için SMS_STATE_SYSTEM bileşende [ayrıntılı günlük kaydını](../../plan-design/hierarchy/about-log-files.md#bkmk_logoptions) etkinleştirin.
